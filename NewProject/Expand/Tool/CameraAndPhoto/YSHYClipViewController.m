...
-(instancetype)initWithImage:(UIImage *)image ClipInfo:(ClipInfo)clipInfo
{
    if(self = [super init])
    {
        ...
        self.clipType = SQUARECLIP; //方形裁剪
        self.radius = ([UIScreen mainScreen].bounds.size.width - 120)/2;//设置 裁剪框的半径
        self.scaleRation = 2;// 图片缩放的最大倍数
        lastScale = 1.0;
    }
    return  self;
}
...
-(UIButton *)backBtn{
    if (!_backBtn) {
        ...
        [_backBtn setFrame:CGRectMake(0, 20, 60, 50)];
        [_backBtn setImageEdgeInsets:UIEdgeInsetsMake(0, 20, 25, 24)];
        [_backBtn setImage:GetImage(@"btn_back_imagePicker.png") forState:UIControlStateNormal];
...
-(UIButton *)clipBtn{
    if (!_clipBtn) {
        ...
        [_clipBtn setTitle:@"使用" forState:UIControlStateNormal];
        _clipBtn.titleLabel.font=[UIFont fontWithName:@"AvenirNext-DemiBold" size:16.0f];
        [_clipBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        _clipBtn.frame=CGRectMake(SCREEN_WIDTH - 32 -10, 20, 32, 24);
...
-(void)clipBtnSelected:(UIButton *)btn
{
    ...
    if (self.clipInfo) {
        ...
        self.clipInfo([self getSmallImage]);
    }
...
-(void)drawClipPath:(ClipType)clipType
{
    ...
    {
        //绘制圆形裁剪区域
        [squarePath  appendPath:[UIBezierPath bezierPathWithArcCenter:self.view.center radius:self.radius startAngle:0 endAngle:2*M_PI clockwise:NO]];
    }
    else
    {
        [squarePath appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(center.x - self.radius, center.y - self.radius, self.radius * 2, self.radius * 2)]];
    }
    squareLayer.path = squarePath.CGPath;
    [_overView.layer addSublayer:squareLayer];
}
