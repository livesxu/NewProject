...
@property (nonatomic, weak) UIImageView *tickImageView;//打钩，暂时不用
...
- (UIImageView *)tickImageView {
    if (!_tickImageView) {
        ...
        tickImageView.image = [UIImage imageNamed:@"ico_eye"];
...
- (void)setMaskViewFlag:(BOOL)maskViewFlag {
    ...
    if (!maskViewFlag) {
        // hidden
        [self.tickImageView setImage:[UIImage imageNamed:@"ico_eye"]];
    }else{
        [self.tickImageView setImage:[UIImage imageNamed:@"ico_eye"]];
    }
