...
- (void) getAllGroupAllPhotos:(BOOL)allPhotos withResource : (groupCallBackBlock ) callBack {
    ...
    ALAssetsLibraryGroupsEnumerationResultsBlock resultBlock = ^(ALAssetsGroup *group, BOOL *stop) {
        if (group) {
            ...
            pickerGroup.groupName = [self changeGroupName:[group valueForProperty:@"ALAssetsGroupPropertyName"]];
...
#pragma mark - 自定义系统相册名称，livesxu 记忆功能同步更改 LGPhotoPickerGroupViewController gotoHistoryGroup
-(NSString *)changeGroupName:(NSString *)oldGroupName{
    
    return [oldGroupName isEqualToString:@"Camera Roll"] ? @"相机胶卷" : [oldGroupName isEqualToString:@"My Photo Stream"] ? @"我的照片流" : oldGroupName ;
}
