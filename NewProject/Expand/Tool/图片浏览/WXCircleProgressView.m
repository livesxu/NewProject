...
+ (instancetype)circleViewShowInView:(UIView *)view {
    ...
    CGFloat x = (width - kProgressWidth ) /  2;
    CGFloat y = (height - kProgressHeight ) /  2;
    if (height > kScreenHeight) {
        y = (kScreenHeight - kProgressHeight) / 2;
    }
    WXCircleProgressView *circle = [[self alloc] initWithFrame:CGRectMake(x, y, kProgressWidth, kProgressHeight)];
